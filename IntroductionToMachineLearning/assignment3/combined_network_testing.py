...
def cross_val_metrics_combined_network(model, X, y, autoencoder, cv=5, repeats=2, task='classification', num_classes=2, generate_graphs=False):
    """
    Performs the Cross Validation on the Combined Neural Network Model
    ...
    autoencoder (autoencoder): The untrained but already created autoencoder for each dataset
    ...
    for _ in range(repeats):
        ...
        for fold in range(cv):
            ...
            # Train autoencoder
            autoencoder.fit(X_train, epochs=1000, learning_rate=0.01)
            ...
            if task == 'classification':
                ...
                model.fit(encoded_X_train, y_train_encoded, epochs=1000, learning_rate=0.01)
                y_test_encoded = one_hot_encode(y_test, num_classes=num_classes)
                score = evaluate_classification(model, encoded_X_test, y_test_encoded)
                if fold == 4 and generate_graphs:
                    plot_confusion_matrix(model, encoded_X_test, y_test_encoded, f"graphs/{dataset_name}/cn")
            else:
                model.fit(encoded_X_train, y_train, epochs=1000, learning_rate=0.01)
                score = evaluate_regression(model, encoded_X_test, y_test)
                if fold == 4 and generate_graphs:
                    plot_regression_metrics(model, encoded_X_test, y_test, f"graphs/{dataset_name}/cn")
...
def load_datasets():
    ...
    datasets['Abalone'] = (X_abalone_normalized, y_normalized, 'regression', 1)
...
if __name__ == "__main__":
    ...
    file = open("output/combined_network_stats.txt", "a")
    for dataset_name, (X, y, task, num) in datasets.items():
        ...
        # Ensure same autoencoder instance is called
        autoencoder = Autoencoder(input_size=X_train.shape[1], hidden_size=10)

        if task == 'classification':
            model = CombinedNetwork(autoencoder, hidden_size=10, output_size=num)
        else:
            model = CombinedNetwork(autoencoder, hidden_size=10, output_size=1)

        mean_score, std_score = cross_val_metrics_combined_network(model, X_train, y_train, autoencoder, cv=5, repeats=2, task=task, num_classes=num)
